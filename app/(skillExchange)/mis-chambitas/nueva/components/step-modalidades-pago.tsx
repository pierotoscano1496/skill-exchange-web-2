...
import { CreditCard, Smartphone, Globe, Banknote, Plus, Trash2, Check, AlertCircle } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export function StepModalidadesPago({ formData, updateFormData, errors }: StepModalidadesPagoProps) {
  ...
  const [nuevaModalidad, setNuevaModalidad] = useState<ModalidadPago>({ tipo: ModalidadPagoTipo.YAPE })
  const [erroresModalidad, setErroresModalidad] = useState<Record<string, string>>({})

  const validarNuevaModalidad = (): boolean => {
    const errores: Record<string, string> = {}
    const yaExiste = formData.modalidadesPago.some((m) => m.tipo === nuevaModalidad.tipo)
    if (yaExiste) {
  ...
  const agregarModalidad = () => {
    if (validarNuevaModalidad()) {
      updateFormData({ modalidadesPago: [...formData.modalidadesPago, nuevaModalidad] })
  ...
  const eliminarModalidad = (index: number) => {
    updateFormData({ modalidadesPago: formData.modalidadesPago.filter((_, i) => i !== index) })
  }

  const obtenerConfigTipo = (tipo: string) => tiposPago.find((t) => t.id === tipo) || tiposPago[0]
  ...
  return (
    <div className="space-y-6">
        ...
        <CardHeader>
          <div className="flex justify-between items-start">
            <div>
              ...
              <CardDescription>Configura las formas de pago que aceptas para tu servicio.</CardDescription>
            </div>
          ...
          {errors.modalidadesPago && <p className="text-sm text-red-500 mt-2">{errors.modalidadesPago}</p>}
        </CardHeader>
        <CardContent>
          {formData.modalidadesPago.length > 0 ? (
            ...
            <div className="text-center p-6 border-2 border-dashed rounded-lg">
              ...
              <h3 className="mt-4 text-lg font-medium">No has agregado modalidades de pago</h3>
      ...
      <Alert>
        ...
        <AlertDescription>
          Chambita cobra una comisión del 10% por cada transacción completada a través de la plataforma. Los pagos se
          procesarán de forma segura.
        </AlertDescription>
      </Alert>
    </div>
  )
