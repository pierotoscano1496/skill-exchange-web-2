    ...
    if (step === 1) {
      ...
      if (formData.modalidadesPago.length === 0) {
        ...
        formData.modalidadesPago.forEach((modalidad, index) => {
          if (modalidad.tipo === ModalidadPagoTipo.YAPE && !modalidad.numeroCelular) {
            newErrors[`modalidad_${index}_celular`] = "El número de celular es requerido para Yape"
          }
          if (modalidad.tipo === ModalidadPagoTipo.TARJETA && !modalidad.cuentaBancaria) {
            newErrors[`modalidad_${index}_cuenta`] = "La cuenta bancaria es requerida para tarjeta"
          }
          if (modalidad.tipo === ModalidadPagoTipo.LINEA && !modalidad.url) {
            newErrors[`modalidad_${index}_url`] = "La URL es requerida para pago en línea"
          }
        })
      }
    } else if (step === 5) {
    ...
    try {
      ...
      const requestBody: ServicioRequestBody = {
        data: {
          ...
          precioMinimo: formData.precioMinimo ? Number.parseFloat(formData.precioMinimo) : undefined,
          precioMaximo: formData.precioMaximo ? Number.parseFloat(formData.precioMaximo) : undefined,
          ...
          disponibilidades: transformarDisponibilidades(formData.disponibilidad),
      ...
      setSubmitError(error instanceof Error ? error.message : "Error inesperado al crear el servicio")
    } finally {
      ...
      {success ? (
        <Card>
          <CardContent className="pt-6 pb-4 text-center">
            ...
            <h2 className="text-xl font-bold mb-2">¡Servicio publicado con éxito!</h2>
            <p className="text-muted-foreground mb-4">
              Tu servicio ha sido publicado correctamente y ya está disponible para que otros usuarios lo encuentren.
            </p>
            <Alert className="bg-muted border-muted-foreground/20 mb-4">
              <AlertDescription>Serás redirigido a la página de tus chambitas en unos segundos...</AlertDescription>
            </Alert>
          </CardContent>
        </Card>
      ) : (
