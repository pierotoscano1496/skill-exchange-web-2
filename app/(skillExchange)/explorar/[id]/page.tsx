...
export default function ServiceDetailPage({ params }: { params: { id: string } }) {
  ...
  const formatPrecio = (servicio: ServicioDetalle) => {
    ...
    switch (servicio.tipoPrecio) {
      ...
      case "rango":
        return `S/ ${servicio.precioMinimo?.toFixed(2) || 0} - S/ ${servicio.precioMaximo?.toFixed(2) || 0}`
      default:
        return `S/ ${servicio.precio.toFixed(2)}`
    }
  }
    ...
    return (
      <div className="container mx-auto py-10">
        ...
        <Alert variant="destructive" className="mb-6">
          ...
          <AlertDescription>
            {error || "No se pudo cargar el servicio. Por favor, intenta nuevamente."}
          </AlertDescription>
        </Alert>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 text-sm text-muted-foreground mb-4">
                    <div className="flex items-center">
                      ...
                      <span>{reviews?.puntajePromedio.toFixed(1) || "Sin calificaciones"}</span>
              ...
              <Tabs defaultValue="details" onValueChange={setActiveTab} className="w-full">
                ...
                <TabsContent value="details" className="pt-4">
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {servicio.disponibilidades.map((disp) => (
                          <div key={disp.id} className="flex items-center p-2 border rounded-md">
                            <Calendar className="h-4 w-4 mr-2 text-muted-foreground" />
                            <span className="capitalize font-medium">{disp.dia}</span>
                    ...
                    <div>
                      <h2 className="text-lg font-semibold mb-2">Métodos de pago aceptados</h2>
                      <div className="flex flex-wrap gap-2">
                        {servicio.modalidadesPago.map((modalidad) => (
                          <div key={modalidad.id}>{getModalidadPagoIcon(modalidad.tipo)}</div>
                        ))}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-2 mb-4">
                      <div className="flex items-center">
                        ...
                        <span className="text-xl font-bold ml-1">{reviews?.puntajePromedio.toFixed(1) || "0.0"}</span>
                      </div>
                      <span className="text-muted-foreground">
                        ({reviews?.comentarios.length || 0} {reviews?.comentarios.length === 1 ? "reseña" : "reseñas"})
                      </span>
                    </div>
                          ...
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <div>
                              <div className="font-medium">
                                {review.nombresComentarista} {review.apellidosComentarista}
                              </div>
                            </div>
                          </div>
                          <div className="flex items-center mb-2">
                            {[...Array(5)].map((_, i) => (
                              <Star
                                key={i}
                                className={`h-4 w-4 ${
                                  i < review.puntaje ? "text-yellow-500 fill-current" : "text-gray-300"
                                }`}
                              />
                            ))}
                          </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-4 mb-6">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">{servicio.proveedor.correo}</p>
                      </div>
                    </div>
                    ...
                    {servicio.proveedor.introduccion && (
                      <div>
                        ...
                        <p className="text-muted-foreground">{servicio.proveedor.introduccion}</p>
                      </div>
                    )}
                      ...
                      <div className="flex flex-wrap gap-2">
                        {servicio.proveedor.perfilLinkedin && (
                          <Button variant="outline" size="sm" asChild>
                            <a href={servicio.proveedor.perfilLinkedin} target="_blank" rel="noopener noreferrer">
                        ...
                        {servicio.proveedor.perfilFacebook && (
                          <Button variant="outline" size="sm" asChild>
                            <a href={servicio.proveedor.perfilFacebook} target="_blank" rel="noopener noreferrer">
                        ...
                        {servicio.proveedor.perfilInstagram && (
                          <Button variant="outline" size="sm" asChild>
                            <a href={servicio.proveedor.perfilInstagram} target="_blank" rel="noopener noreferrer">
                        ...
                        {servicio.proveedor.perfilTiktok && (
                          <Button variant="outline" size="sm" asChild>
                            <a href={servicio.proveedor.perfilTiktok} target="_blank" rel="noopener noreferrer">
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <h2 className="font-semibold text-lg">{getNombreProveedor(servicio)}</h2>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    ...
                    <span className="font-semibold">{formatPrecio(servicio)}</span>
                  </div>
                ...
                <ContactServiceForm servicio={servicio} onSuccess={() => router.push("/explorar")} />
          ...
          <Card>
            <CardContent className="p-6">
              ...
              <div className="space-y-2">
                {servicio.modalidadesPago.map((modalidad) => (
                  <div key={modalidad.id} className="flex items-center justify-between">
                    <div>{getModalidadPagoIcon(modalidad.tipo)}</div>
                    <div className="text-sm text-muted-foreground">
                      {modalidad.tipo === "yape" && modalidad.numeroCelular && `Cel: ${modalidad.numeroCelular}`}
                      {modalidad.tipo === "tarjeta" && "Tarjeta de crédito/débito"}
