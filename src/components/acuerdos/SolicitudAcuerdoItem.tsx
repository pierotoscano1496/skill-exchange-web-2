...
type Params = {
    ...
    onOpenModalEnviarConstancia: (match: MatchServicioProveedorDetailsResponse) => void;
}

export default ({ match, onOpenViewProfile, onOpenModalEnviarConstancia }: Params) => {
    return (
            ...
            <div className="container">
                ...
                <div className="item flex-grow-1">
                    {match.estado === "pendiente-pago" &&
                        <button className="btn-secondary" onClick={() => onOpenModalEnviarConstancia(match)}>Enviar constancia</button>}
                    {/* {match.estado === "ejecucion" && <button className="btn-info" onClick={() =>}>Enviar mensaje</button>} */}
                    {match.estado === "finalizado" &&
                        <a className="link-button btn-primary" href={`/servicio/details/${match.servicio.id}`}>Comentar y puntuar</a>}
