...
export default memo(({ myself, text, resourceUrl }: Props) => {
    ...
    return (
        <div className={`${bubbleStyles.bubbleMessage} ${myself ? bubbleStyles.myself : bubbleStyles.themself}`}>
            <span>{text}</span>
            {resourceUrl &&
                ((source === resourceUrl) ?
                    <img loading="lazy" src={resourceUrl} alt="Archivo"
                        data-file="image" />
                    :
                    <a href={resourceUrl}>
                        <Image src={source}
