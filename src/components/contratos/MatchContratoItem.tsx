...
import {
  converDateTimeToStandarString,
  converLocalDateTimeToStandarString,
...
type Params = {
  ...
  onOpenModalConfirmarFinalizacion: (
    match: MatchServicioDetailsResponse
  ) => void;
};

const MatchContratoItem = ({
  match,
  onOpenViewProfile,
  onOpenModalConfirmarConstancia,
  onOpenModalConfirmarFinalizacion,
}: Params) => {
  return (
      ...
      <div className="container">
        <div className="item flex-grow-3">
          <p>
            ...
            <button
              className="btn-link"
              onClick={() => onOpenViewProfile(match.cliente)}
          ...
          {match.estado === "pendiente-pago" && (
            <p className="text-secondary">Pago pendiente</p>
          )}
          {match.estado === "ejecucion" && (
            <>
              ...
              <p>
                Iniciado el{" "}
                {converLocalDateTimeToStandarString(match.fechaInicio)}
              </p>
            </>
          )}
        ...
        <div className="item flex-grow-1">
          {match.estado === "pendiente-pago" && (
            <button
              ...
              onClick={() => onOpenModalConfirmarConstancia(match)}
            >
          ...
          {match.estado === "ejecucion" && (
            <button
              className="btn-info"
              onClick={() => onOpenModalConfirmarFinalizacion(match)}
            >
              Finalizar servicio
            </button>
          )}
...
MatchContratoItem.displayName = "MatchContratoItem";

export default MatchContratoItem;
