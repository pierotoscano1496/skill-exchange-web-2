...
export const getConversationById = async (idConversation: string) => {
    const resp = await (await getBackendInstanceAuth()).get(`chat/${idConversation}`);
    return resp.data as MensajeChat;
}

export const getConversationWithUser = async (idUsuario: string) => {
    const resp = await (await getBackendInstanceAuth()).get(`chat/with/${idUsuario}`);
    return resp.data as MensajeChat;
}

export const getConversationsFromUserLogged = async () => {
    const resp = await (await getBackendInstanceAuth()).get("chat/own");
    return resp.data as MensajeChat[];
}
...
export const sendContactMessage = async (mensajeChat: FirstMessageChatBody) => {
    ...
    const resp = await (await getBackendInstanceAuth()).post("chat", mensajeChat);
    return resp.data as MensajeChat;
}

export const getChats = async (idReceptor: string) => {
    const resp = await (await getBackendInstanceAuth()).get(`chat/with/${idReceptor}`);
    return resp.data as MensajeChat;
}

export const getChatsNoMessages = async (idReceptor: string) => {
    const resp = await (await getBackendInstanceAuth()).get(`chat/with-no-messages/${idReceptor}`);
    return resp.data as MensajeChat;
}

export const saveChatFile = async (idConversation: string, formDataFile: FormData) => {
    const resp = await (await getBackendInstanceAuthForms()).put(`chat-resources/upload/${idConversation}`, formDataFile);
    return resp.data as string;
}

export const enviarMensajeByForm = async (idConversation: string, message: MessageBody) => {
    const resp = await (await getBackendInstanceAuth()).patch(`chat/${idConversation}/send`, message);
