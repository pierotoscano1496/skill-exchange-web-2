...
export const logoutUsuario = async () => {
  const resp = await (await getBackendInstanceAuth()).post("usuario/logout");
  return resp.data as string;
};

export const registrarUsuarioDatos = async (
...
export const asignarSkillsToUsuario = async (
  idUsuario: string,
  skillsToAsignar: AsignacionSkillToUsuarioRequest[]
) => {
  const resp = await (
    await getBackendInstanceAuth()
  ).patch(`usuario/skills/${idUsuario}`, skillsToAsignar);
